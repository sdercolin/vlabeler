...
if (wavFile.getParentFile().getAbsolutePath() !== projectRootDirectory.getAbsolutePath()) {
    error({
        en: `File ${wavFilePath} is not in the sample directory of this project.`,
        zh: `文件 ${wavFilePath} 不在该项目的采样目录中。`,
        ja: `ファイル ${wavFilePath} はこのプロジェクトのサンプルディレクトリにありません。`,
        ko: `파일 ${wavFilePath}이(가) 이 프로젝트의 샘플 폴더에 없습니다.`
    })
}
...
let labFile = projectRootDirectory.resolve(moduleName + '.lab')
let entries = []
if (labFile.exists()) {
...
let module = new Module(moduleName, projectRootDirectory.getAbsolutePath(), entries, 0, labFile.getAbsolutePath())
modules.push(module)
