...
let dictLines = splitLine(params["dictionary"]).map(x => parseLine(x))
...
for (const note of notes) {
    ...
    if (phonemes.length === 1) {
        ...
        if (phonemes.length === 2) {
            ...
            let vowelDivisions = phonemes.length - 2
            let vowelLength = (vowelEnd - vowelStart) / vowelDivisions
            for (let i = 0; i < vowelDivisions; i++) {
                let start = vowelStart + i * vowelLength
                let end = start + vowelLength
                let phoneme = phonemes[i + 2]
                created = new Entry(sample, phoneme, start, end, [], [])
                if (i < vowelDivisions - 1) {
                    entries.push(created)
                    log("push new: " + JSON.stringify(created))
                } else {
                    last = created
