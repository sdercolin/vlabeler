...
fun Strings.zhHans(): String? = when (this) {
    ...
    StarterNewInputFilePlaceholder -> "（如留空，将根据标注器来创建默认模板）"
    StarterNewInputFileDisabledByLabeler -> "当前选择的标注器不需要输入文件"
    StarterNewInputFileDisabledByPlugin -> "当前选择的模板生成器不需要输入文件"
    StarterNewEncoding -> "编码"
    StarterNewAutoExport -> "自动导出"
    StarterNewAutoExportHelp -> "保存时，自动将工程覆盖导出到输入文件（如未指定输入文件，将自动导出到由标注器指定的位置）"
    StarterNewWarningSelfConstructedLabelerWithTemplatePlugin ->
        "您正在尝试在一个管理多个子工程的标注器上使用模板生成器。" +
    ...
    InvalidCreatedProjectException -> "新建的工程为无效工程。请联系标注器或插件的作者以获取更多信息。"
    ...
    LoadProjectWarningLabelerCreated -> "从工程文件中安装了新的标注器 `%s`。"
    LoadProjectWarningLabelerUpdated -> "从工程文件中安装了标注器 `%1\$s` 的版本更新 `%2\$s`。"
