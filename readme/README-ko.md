...
- `Q`/`W`/`E`/`R`/... : &nbsp;해당하는 마커를 현재의 커서 위치로 이동. <br>마커의 순서는 라벨러가 정의해요.
    - 단일 편집 모드에서는 엔트리의 모든 마커에 적용돼요.
    - 다중 편집 모드에서는 `Q`와 `W`만 사용할 수 있으며, 각각 커서 위치 엔트리의 왼쪽·오른쪽 경계를 옮겨요. 또한 `단축키`에서 `현재 엔트리의 왼쪽/오른쪽 경계를 커서 위치에 놓기`(기본 키 없음)에 키를 지정하면, 커서 위치와 관계없이 현재 엔트리의 경계를 옮길 수 있어요.
...
&nbsp;또한 `설정` -> `환경설정` -> `에디터` -> `클릭하여 엔트리로 이동`(기본값 꺼짐)을 활성화하면, 캔버스에서 엔트리의 본체를 클릭해 해당 엔트리를 현재 엔트리로 전환할 수 있어요. 경계나 마커를 클릭하면 드래그가 시작되며 현재 엔트리는 바뀌지 않아요.
...
&nbsp;vLabeler는 커스텀 라벨러, 플러그인, 앱 설정, 로그 등의 유저 데이터를 앱 폴더에 저장해요. 기본 위치는 `~/vLabeler` (macOS에서는 `~/Library/vLabeler`)입니다.

&nbsp;앱을 실행하기 전에 환경 변수 `VLABELER_APP_DIR`를 폴더의 절대 경로로 설정하면, 이 위치를 변경할 수 있어요. 폴더가 없으면 자동으로 만들어져요. 값이 유효하지 않으면 기본 위치가 사용됩니다.
