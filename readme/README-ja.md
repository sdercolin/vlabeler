...
- `Q`/`W`/`E`/`R`/... : 現在のカーソル位置に対応するパラメータラインを移動します。順序はラベラーで定義されています。
    - 単一エントリ編集モードでは、エントリのすべてのパラメータラインに適用されます。
    - 複数エントリ編集モードでは `Q` と `W` のみが有効で、それぞれカーソル位置のエントリの左右の境界を移動します。
      また、`ショートカット` で `現在のエントリの左/右境界をカーソル位置に設定`（デフォルトのキーなし）にキーを割り当てると、
      カーソル位置に関係なく現在のエントリの境界を移動できます。
...
ピン留めされたエントリリストでは、複数のエントリを選択して一括編集できます。

- `Ctrl` + クリック：エントリを選択に追加、または選択から削除します
- `Shift` + クリック：範囲内のエントリを選択します

macOS をご使用の場合、上記の `Ctrl` は `Command` を指します。

選択したエントリを右クリックすると、以下の一括操作が利用できます。

- 選択したエントリを完了/未完了にする
- 選択したエントリにスターマークを付ける/外す
- 選択したエントリのタグを設定する
- 選択したエントリを削除する

修飾キーを押さずにクリックすると、選択が解除されます。一括操作は 1 回の`取り消し`で元に戻せます。
...
- `vLabeler` プロジェクトの下にサブプロジェクトを構築・管理（追加/名前変更/削除）する方法
