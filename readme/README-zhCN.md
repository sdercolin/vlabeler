...
- `Q`/`W`/`E`/`R`/... ：将对应的参数线移动到当前光标位置。顺序在标注器中定义。
    - 在单条目编辑模式下，可用于该条目的所有参数线。
    - 在多条目编辑模式下，仅 `Q` 和 `W` 可用，分别将光标所在条目的左、右边界移动到光标位置。你也可以在 `键位映射` 中为
      `将当前条目的左/右边界设置到光标位置` 操作（无默认按键）绑定按键，以便不受光标位置影响地移动当前条目的边界。
...
在固定条目列表中，您可以选择多个条目并对它们进行批量编辑：

- `Ctrl` + 单击：将条目加入选择，或从选择中移除
- `Shift` + 单击：选择一个范围内的条目

请注意，如果您使用的是 macOS，则以上的 `Ctrl` 指的是 `Command`。

右键单击选中的条目，可以使用以下批量操作：
...
不按修饰键的单击将清除选择。批量操作可以通过一次 `撤销` 来还原。
